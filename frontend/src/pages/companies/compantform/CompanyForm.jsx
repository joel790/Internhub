...
const CompanyForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    formData.append("name", companyInfo.companyName);
    formData.append("location", companyInfo.location);
    formData.append("slogan", companyInfo.slogan);
    ...
    formData.append("jobTitle", companyInfo.jobTitle);
    formData.append("contactNumber", companyInfo.phone);
    formData.append("website", companyInfo.twitter);
    formData.append("license", documents.companyDocument);
    formData.append("logo", documents.companyLogo);

    try {
      const response = await axios.post(`http://localhost:5000/api/student/apply/${planId}`, formData, {
        headers: {
          "Content-Type": "multipart/form-data"
        }
      });
      console.log(response.data);
      // Handle successful application, e.g., show a success message or redirect
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col px-10 mt-5 py-6">
        ...
        <div className="flex sm:flex-col md:flex-row gap-5 w-full mt-6">
          <div className="flex flex-col gap-4 w-full sm:w-full md:w-1/2">
            <label className="font-bold text-xl" htmlFor="companyName">Company Name</label>
        ...
        <div className="flex flex-col gap-4 w-full mt-5">
          ...
          <textarea className="px-4 py-1 h-28 border border-neutral-400 rounded-lg" placeholder="description" name="description" value={companyInfo.description} onChange={handleChange} />
        ...
        <div className="flex flex-col gap-4 w-full mt-6">
          <label className="font-bold text-xl" htmlFor="email">Email Address</label>
          <input className="px-4 py-1 border border-neutral-400 rounded-lg" type="email" placeholder="email" name="email" value={companyInfo.email} onChange={handleChange} />
        ...
        <hr className="w-full border-blue-700 "></hr>
        <div className="flex flex-col gap-4 w-full mt-6">
          <label className="font-bold text-xl" htmlFor="companyDocument">Company Document</label>
        ...
        <div className="flex flex-col gap-4 w-full ">
          <label className="font-bold text-xl" htmlFor="companyLogo">Company Logo</label>
          <input className="px-4 py-1 border border-neutral-400 rounded-lg" type="file" name="companyLogo" onChange={handleFileChange} />
